...
const ToolButton: React.FC<{ 
    ...
    <button
        ...
        className={`group flex-1 flex items-center justify-center gap-3 p-4 text-sm font-semibold rounded-xl transition-all duration-300 transform hover:scale-105 disabled:scale-100 shadow-lg hover:shadow-xl ${
            disabled 
                ? 'bg-gray-300 text-gray-500 cursor-not-allowed' 
                : `${gradient} text-white hover:shadow-2xl`
        }`}
    >
        {children}
    </button>
...
export const AiTools: React.FC<AiToolsProps> = ({
    ...
    return (
        <div className={`mb-8 ${isDarkMode ? 'bg-gradient-to-r from-blue-900/20 to-purple-900/20 border-blue-700' : 'bg-gradient-to-r from-blue-50 to-purple-50 border-blue-300'} border rounded-2xl shadow-lg transition-all duration-500`}>
            <details className="group">
                ...
                <div className="px-6 pb-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {isGenerating ? (
                            <div className="sm:col-span-3 flex items-center justify-center gap-3 p-6 text-lg font-medium text-blue-600 bg-blue-50 rounded-xl border border-blue-200">
                           ...
                           <>
                             ...
                             >
                                <TargetIcon className="w-5 h-5 group-hover:scale-110 transition-transform" />
                                <span>Analyze</span>
                             </ToolButton>
                             <ToolButton 
